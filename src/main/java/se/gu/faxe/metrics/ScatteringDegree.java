...
public class ScatteringDegree {
    ...
    /**
     * Calculate Scattering Degree: total number of all annotations directly referencing the feature (i.e., in-file,
     * folder, and file annotations referencing it).
     *
    ...
    /**
     * Calculate Average Scattering Degree over all Features: total number of all annotations referencing all features
     * (i.e., in-file, folder, and file annotations referencing it).
     *
     * @param fullAssetTree Asset tree generated by FAXE to traverse for metrics
     * @param searchedPath  Sub-Tree to search for data
     * @return Scattering Degree of given feature in given path.
     * @throws IOException in case searchedPath does not exist in fullAssetTree.
     */
    public static double calculateAverageSD(TreeNode<Asset> fullAssetTree, File searchedPath) throws IOException {
        return calculateAverageSD(fullAssetTree, searchedPath, false);
    }
     ...
     * @throws IOException in case searchedPath does not exist in fullAssetTree.
     */
    public static double calculateAverageSD(TreeNode<Asset> fullAssetTree, File searchedPath, boolean printFoundLocation) throws IOException {
        /**
        ...
        /**
         * CALCULATE SD PER FEATURE IN SCOPE
         */
        double sumSD = 0.0;
        for(String feature : uniqueFeatures){
            Feature searchFeature = new Feature(feature);
            int SD = calculateSD(fullAssetTree, searchedPath, searchFeature/*, printFoundLocation*/);
            sumSD += SD;
            if (printFoundLocation) {
                System.out.println("calculateAverageSD for feature \"" + searchFeature + "\" has SD=" + SD);
            }
        }

        double avgSD = sumSD / ((double) uniqueFeatures.size());
        if (printFoundLocation) {
            System.out.println("calculateAverageSD total average SD = " + avgSD +" (sumSD=" +sumSD +" ; uniqueFeatures.size=" +uniqueFeatures.size() +")");
        }
        return avgSD;
    }
