...
public class WalletBalanceWidgetProvider extends AppWidgetProvider {
    ...
    public static void updateWidgets(final Context context, final Coin balance,	//&line[ExchangeRates]
    ...
    private static void updateWidgets(final Context context, final AppWidgetManager appWidgetManager,
            final int[] appWidgetIds, final Coin balance, final @Nullable ExchangeRate exchangeRate) {	//&line[ExchangeRates]
        for (final int appWidgetId : appWidgetIds) {
            final Bundle options = getAppWidgetOptions(appWidgetManager, appWidgetId);
            updateWidget(context, appWidgetManager, appWidgetId, options, balance, exchangeRate);	//&line[ExchangeRates]
        }
    }
    ...
    private static void updateWidget(final Context context, final AppWidgetManager appWidgetManager,
            ...
            final @Nullable ExchangeRate exchangeRate) {	//&line[ExchangeRates]
        final WalletApplication application = (WalletApplication) context.getApplicationContext();
        ...
        if (appWidgetOptions != null) {
            ...
            views.setViewVisibility(R.id.widget_button_request, minWidth > 300 ? View.VISIBLE : View.GONE);	//&line[RequestCoins]
            //&begin[SendCoins]
            views.setViewVisibility(R.id.widget_button_send, minWidth > 300 ? View.VISIBLE : View.GONE);
            views.setViewVisibility(R.id.widget_button_send_qr, minWidth > 200 ? View.VISIBLE : View.GONE);	//&line[QRCode]
            //&end[SendCoins]
        }
