...
public abstract class InputParser {
    ...
    public abstract static class StringInputParser extends InputParser {
        ...
        public void parse() {
            if (input.startsWith("BITCOIN:-")) {
                try {
                    final byte[] serializedPaymentRequest = Qr.decodeBinary(input.substring(9));	//&line[QRCode]
                ...
                try {
                    handlePrivateKey(DumpedPrivateKey.fromBase58(Constants.NETWORK_PARAMETERS, input));	//&line[base58]
                } catch (AddressFormatException x) {
                    try {
                        handlePrivateKey(BIP38PrivateKey.fromBase58(Constants.NETWORK_PARAMETERS, input));	//&line[base58]
                    } catch (final AddressFormatException x2) {
                        try {
                            handlePaymentIntent(PaymentIntent
                                    .fromAddress(Address.fromBase58(Constants.NETWORK_PARAMETERS, input), null));	//&line[base58]
    ...
    public static PaymentIntent parsePaymentRequest(final byte[] serializedPaymentRequest)
        ...
        try {
            ...
            final String paymentUrl = paymentSession.getPaymentUrl();	//&line[PaymentURL]

            final byte[] merchantData = paymentSession.getMerchantData();

			//&begin[Codecs]
            final byte[] paymentRequestHash = Hashing.sha256().hashBytes(serializedPaymentRequest).asBytes();	//&line[TBIP75]
			//&end[Codecs]

            //&begin[PaymentURL]
			//&begin[Codecs]
            final PaymentIntent paymentIntent = new PaymentIntent(PaymentIntent.Standard.BIP70, pkiName, pkiCaName,	//&line[BIP70]
                    outputs.toArray(new PaymentIntent.Output[0]), memo, paymentUrl, merchantData, null,
                    paymentRequestHash);	//&line[TBIP75]
			//&end[Codecs]
