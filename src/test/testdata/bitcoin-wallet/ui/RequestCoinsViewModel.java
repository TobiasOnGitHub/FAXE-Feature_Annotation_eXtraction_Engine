...
public class RequestCoinsViewModel extends AndroidViewModel {
    ...
    public final FreshReceiveAddressLiveData freshReceiveAddress;	//&line[SenderAddress]
    private final ConfigOwnNameLiveData ownName;	//&line[OwnName]
    ...
    public final MutableLiveData<String> bluetoothMac = new MutableLiveData<>();	//&line[Bluetooth]
    //&linde[QRCode]
    ...
    public RequestCoinsViewModel(final Application application) {
        ...
        this.ownName = new ConfigOwnNameLiveData(this.application);	//&line[OwnName]
        this.exchangeRate = new SelectedExchangeRateLiveData(this.application);	//&line[ExchangeRate]
        //&begin[QRCode]
    ...
    private void maybeGenerateQrCode() {
        final Address address = freshReceiveAddress.getValue();	//&line[SenderAddress]
        if (address != null) {
            AsyncTask.execute(new Runnable() {
                @Override
                public void run() {
                    qrCode.postValue(
                            Qr.bitmap(uri(address, amount.getValue(), ownName.getValue(), bluetoothMac.getValue())));	//&line[Bluetooth]
                }
            });
        }
    }
    ...
    private void maybeGeneratePaymentRequest() {
        ...
        if (address != null) {
            ...
            paymentRequest.setValue(PaymentProtocol.createPaymentRequest(Constants.NETWORK_PARAMETERS,
                    amount.getValue(), address, ownName.getValue(), paymentUrl, null).build().toByteArray());	//&line[OwnName]
            //&end[PaymentURL]
            //&end[Bluetooth]
        }
    }
    ...
    private void maybeGenerateBitcoinUri() {
        final Address address = freshReceiveAddress.getValue();	//&line[SenderAddress]
        if (address != null) {
            bitcoinUri.setValue(Uri.parse(uri(address, amount.getValue(), ownName.getValue(), null)));	//&line[OwnName]
        }
    }
