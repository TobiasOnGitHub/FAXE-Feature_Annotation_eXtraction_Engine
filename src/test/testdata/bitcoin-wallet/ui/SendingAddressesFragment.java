...
public final class SendingAddressesFragment extends FancyListFragment {
    ...
    private void handlePasteClipboard() {
        ...
        if (address == null) {
            ...
            final DialogBuilder dialog = new DialogBuilder(activity);
            dialog.setTitle(R.string.address_book_options_paste_from_clipboard_title);
            dialog.setMessage(R.string.address_book_options_paste_from_clipboard_own_address); //&line[OwnAddress]
            dialog.singleDismissButton(null);
            dialog.show();
        }
    }
    ...
    private void handleSend(final String address, final String label) {
        SendCoinsActivity.start(activity, PaymentIntent.fromAddress(address, label));	//&line[SendCoins]
    }
    ...
    private Address getAddressFromPrimaryClip() {
        ...
        if (clipDescription.hasMimeType(ClipDescription.MIMETYPE_TEXT_PLAIN)) {
            ...
            try {
				//&begin[Codecs]
                return Address.fromBase58(Constants.NETWORK_PARAMETERS, clipText.toString().trim());	//&line[base58]
				//&end[Codecs]
