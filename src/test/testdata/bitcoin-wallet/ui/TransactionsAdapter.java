...
public class TransactionsAdapter extends ListAdapter<TransactionsAdapter.ListItem, RecyclerView.ViewHolder> {
    public static List<ListItem> buildListItems(final Context context, final List<Transaction> transactions,
            ...
            final @Nullable Map<String, AddressBookEntry> addressBook, final MonetaryFormat format,	//&line[AddressBook]
        ...
        for (final Transaction tx : transactions)
            items.add(new ListItem.TransactionItem(context, tx, wallet, addressBook, noCodeFormat, maxConnectedPeers,	//&line[AddressBook]
            ...
            if (warningItem.type == WarningType.BACKUP) {
                if (getItemCount() == 2 /* 1 transaction, 1 warning */) {
                    ...
                    warningHolder.messageView.setText(
                            Html.fromHtml(context.getString(R.string.wallet_disclaimer_fragment_remind_backup)));	//&line[ShowDisclaimer]
                    //&end[BackupReminder]
                }
            //&end[BackupWallet]
