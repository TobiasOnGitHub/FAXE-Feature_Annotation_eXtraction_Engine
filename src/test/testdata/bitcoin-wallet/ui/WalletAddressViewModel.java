...
public class WalletAddressViewModel extends AndroidViewModel {
    ...
    public final ConfigOwnNameLiveData ownName;	//&line[OwnName]
    public final MediatorLiveData<Bitmap> qrCode = new MediatorLiveData<>();	//&line[QRCode]
    ...
    public WalletAddressViewModel(final Application application) {
        ...
        this.qrCode.addSource(currentAddress, new Observer<Address>() {	//&begin[QRCode]
            @Override
    ...
    private void maybeGenerateQrCode() {
        ...
        if (address != null) {
            AsyncTask.execute(new Runnable() {
                @Override
                public void run() {
                    qrCode.postValue(Qr.bitmap(uri(address, ownName.getValue())));	//&line[OwnName]
                }
            });
        }
    }
    ...
    private void maybeGenerateBitcoinUri() {
        final Address address = currentAddress.getValue();
        if (address != null) {
            bitcoinUri.setValue(Uri.parse(uri(address, ownName.getValue())));	//&line[OwnName]
        }
    }
        ...
        protected void load() {
            ...
            AsyncTask.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    postValue(wallet.currentReceiveAddress());	//&line[SenderAddress]
                }
            });
        }
