...
public class RestoreWalletFromExternalDialogFragment extends DialogFragment {
    ...
    private EditText passwordView;	//&line[SetPIN]
    ...
    public Dialog onCreateDialog(final Bundle savedInstanceState) {
        ...
        passwordView = (EditText) view.findViewById(R.id.import_keys_from_content_dialog_password);	//&line[SetPIN]
        ...
        builder.setNegativeButton(R.string.button_cancel, new OnClickListener() {
            @Override
            public void onClick(final DialogInterface dialog, final int which) {
                passwordView.setText(null); // get rid of it asap	//&line[SetPIN]
                activity.finish();
            }
        });
    ...
    private void handleRestore(final String password) {	//&line[SetPIN]
        try {
            final InputStream is = contentResolver.openInputStream(backupUri);
            final Wallet restoredWallet = restoreWalletFromEncrypted(is, password);	//&line[SetPIN]
    ...
    private Wallet restoreWalletFromEncrypted(final InputStream cipher, final String password) throws IOException {	//&line[SetPIN]
        final BufferedReader cipherIn = new BufferedReader(new InputStreamReader(cipher, StandardCharsets.UTF_8));
        final StringBuilder cipherText = new StringBuilder();
        Io.copy(cipherIn, cipherText, Constants.BACKUP_MAX_CHARS);	//&line[BackupWallet]
        cipherIn.close();

        final byte[] plainText = Crypto.decryptBytes(cipherText.toString(), password.toCharArray());	//&line[SetPIN]
        final InputStream is = new ByteArrayInputStream(plainText);

		//&begin[Codecs]
        return WalletUtils.restoreWalletFromProtobufOrBase58(is, Constants.NETWORK_PARAMETERS);	//&line[base58]
		//&end[Codecs]
    }
        ...
        public Dialog onCreateDialog(final Bundle savedInstanceState) {
            ...
            dialog.setNeutralButton(R.string.button_ok, new DialogInterface.OnClickListener() {
                ...
                public void onClick(final DialogInterface dialog, final int id) {
                    BlockchainService.resetBlockchain(activity);	//&line[ResetBlockChain]
                    activity.finish();
                }
            });
