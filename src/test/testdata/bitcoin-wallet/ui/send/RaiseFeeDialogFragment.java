...
public class RaiseFeeDialogFragment extends DialogFragment {
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        viewModel.getDynamicFees().observe(this, new Observer<Map<FeeCategory, Coin>>() {
            ...
            public void onChanged(final Map<FeeCategory, Coin> dynamicFees) {
                ...
                feeRaise = dynamicFees.get(FeeCategory.PRIORITY).multiply(size).divide(1000);	//&line[Priority]
                updateView();
            }
        });
    ...
    public Dialog onCreateDialog(final Bundle savedInstanceState) {
        ...
        dialog.setOnShowListener(new OnShowListener() {
            ...
            public void onShow(final DialogInterface d) {
                ...
                passwordView.addTextChangedListener(textWatcher);	//&line[SetPIN]
    ...
    private void handleGo() {
        ...
        if (wallet.isEncrypted()) {
            ...
            }.deriveKey(wallet, passwordView.getText().toString().trim());	//&line[SetPIN]
    ...
    private void doRaiseFee(final KeyParameter encryptionKey) {
        ...
        try {
            ...
            BlockchainService.broadcastTransaction(activity, transactionToSend);	//&line[BlockchainSync]
    ...
    private void updateView() {
        ...
        final boolean needsPassword = wallet.isEncrypted();	//&line[SetPIN]

        if (feeRaise == null) {
            messageView.setText(R.string.raise_fee_dialog_determining_fee);
            passwordGroup.setVisibility(View.GONE);	//&line[SetPIN]
        } else if (findSpendableOutput(wallet, transaction, feeRaise) == null) {
            messageView.setText(R.string.raise_fee_dialog_cant_raise);
            passwordGroup.setVisibility(View.GONE);	//&line[SetPIN]
        } else {
            messageView.setText(getString(R.string.raise_fee_dialog_message, config.getFormat().format(feeRaise)));	//&line[Denomination]
            passwordGroup.setVisibility(needsPassword ? View.VISIBLE : View.GONE);	//&line[SetPIN]
        }

        if (state == State.INPUT) {
            positiveButton.setText(R.string.raise_fee_dialog_button_raise);
            positiveButton.setEnabled((!needsPassword || passwordView.getText().toString().trim().length() > 0)	//&line[SetPIN]
