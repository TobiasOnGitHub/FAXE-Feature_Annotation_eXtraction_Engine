...
public class WalletUtils {
    ...
    public static Wallet restoreWalletFromAutoBackup(final Context context) {
        try (final InputStream is = context.openFileInput(Constants.Files.WALLET_KEY_BACKUP_PROTOBUF)) {	//&line[BackupWallet]
            final Wallet wallet = new WalletProtobufSerializer().readWallet(is, true, null);
            if (!wallet.isConsistent())
                throw new Error("inconsistent backup");

            BlockchainService.resetBlockchain(context);	//&line[ResetBlockChain]
            log.info("wallet restored from backup: '" + Constants.Files.WALLET_KEY_BACKUP_PROTOBUF + "'");	//&line[BackupWallet]
            return wallet;
    ...
    public static Wallet restoreWalletFromProtobufOrBase58(final InputStream is,
        ...
        is.mark((int) Constants.BACKUP_MAX_CHARS);	//&line[BackupWallet]
    ...
    public static void writeKeys(final Writer out, final List<ECKey> keys) throws IOException {
        ...
        for (final ECKey key : keys) {
			//&begin[Codecs]
            out.write(key.getPrivateKeyEncoded(Constants.NETWORK_PARAMETERS).toBase58());	//&line[base58]
			//&end[Codecs]
    ...
    public static List<ECKey> readKeys(final BufferedReader in, final NetworkParameters expectedNetworkParameters)
        ...
        try {
            ...
            while (true) {
				...
				//&begin[Codecs]
                final ECKey key = DumpedPrivateKey.fromBase58(expectedNetworkParameters, parts[0]).getKey();	//&line[base58]
				//&end[Codecs]
